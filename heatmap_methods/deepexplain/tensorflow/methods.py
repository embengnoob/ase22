...
class AttributionMethod(object):
    ...
    """ 
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase=None):
        self.T = T
        self.xs = xs
        self.xs_tensor = xs_tensor
        self.y = y
        self.tape = tape_obejct
        self.model = model
...
class GradientBasedMethod(AttributionMethod):
    ...
    def get_symbolic_attribution(self):
        return self.xs_tensor * self.tape.gradient(self.y, self.xs_tensor)
...
class DummyZero(GradientBasedMethod):
    ...
    def get_symbolic_attribution(self,):
        ...
        return self.tape.gradient(self.y, self.xs_tensor)
...
def threshold(x, q):
        if len(x.shape.as_list()) > 3:
            thresh = tfp.stats.percentile(x, q, axis=[1,2,3], keepdims=True)
        else:
...
class RectifiedGradient(GradientBasedMethod):
    ...
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, percentile=98):
        super(RectifiedGradient, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase)
        global q
        q = percentile
    ...
    def get_symbolic_attribution(self):
        ...
        #     self.tape.gradient(ys=self.T, xs=self.X),
    ...
    def nonlinearity_grad_override(cls, op, grad):
        ...
        return tf.compat.v1.where(thresh < activation_grad, grad, tf.zeros_like(grad))
    ...
    def conv2d_grad_override(cls, op, grad):
        ...
        if op.get_attr('padding') == b'SAME':
            ...
            shape = tf.shape(input=grad)
            mask = tf.ones([shape[0], shape[1] - 2, shape[2] - 2, shape[3]])
            mask = tf.pad(tensor=mask, paddings=[[0,0],[1,1],[1,1],[0,0]])
            grad = grad * mask

        input_grad = tf.nn.conv2d_transpose(output_shape=tf.shape(input=op.inputs[0]), filters=op.inputs[1], input=grad, strides=op.get_attr('strides'), padding=op.get_attr('padding'))
        filter_grad = tf.compat.v1.nn.conv2d_backprop_filter(op.inputs[0], tf.shape(input=op.inputs[1]), grad, op.get_attr('strides'), op.get_attr('padding'))
...
class RectifiedGradientConst(GradientBasedMethod):
    ...
    def nonlinearity_grad_override(cls, op, grad):
        ...
        return tf.compat.v1.where(t < activation_grad, grad, tf.zeros_like(grad))
...
class RectifiedGradientPRR(RectifiedGradient):
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, percentile=98):
        super(RectifiedGradientPRR, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, percentile)
    
    @classmethod
    def maxpool_grad_override(cls, op, grad):
        
        z = tf.nn.avg_pool2d(input=op.inputs[0], ksize=op.get_attr('ksize'), strides=op.get_attr('strides'), padding=op.get_attr('padding')) + 1e-10
        s = grad / z
        c = gen_nn_ops.avg_pool_grad(tf.shape(input=op.inputs[0]), s, op.get_attr('ksize'), op.get_attr('strides'), op.get_attr('padding'))
...
class GuidedBackpropagation(GradientBasedMethod):
    ...
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase):
        super(GuidedBackpropagation, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase)
    ...
    def nonlinearity_grad_override(cls, op, grad):
        ...
        return tf.compat.v1.where(op.inputs[0] > 0, tf.nn.relu(grad), tf.zeros_like(grad))
...
class SmoothGrad(GradientBasedMethod):
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, noise_level=0.1, steps=50):
        super(SmoothGrad, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase)
    ...
    def run(self):
        ...
        for _ in range(self.steps):
            ...
            xs_tensor_mod = tensor(xs_mod)
            with tf.GradientTape() as smoothgrad_tape:
                # Forward pass
                smoothgrad_tape.watch(xs_tensor_mod)
                # y_pred = self_driving_car_model(xs)
                y_pred = self.model(xs_tensor_mod)
                # cprintf(f'{(tape.gradient(y_pred, self_driving_car_model.trainable_variables))}', 'l_red')
            gradients = smoothgrad_tape.gradient(y_pred, xs_tensor_mod)
            # _attr = self.session_run(attributions, xs_mod)
            # cprintf(f'{self.tape.gradient(self.y, tensor(xs_mod))}', 'l_red')
            _attr = gradients
...
class IntegratedGradients(GradientBasedMethod):
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, steps=50, baseline=None):
        super(IntegratedGradients, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase)
        self.steps = steps
    ...
    def run(self):
        ...
        for alpha in list(np.linspace(1. / self.steps, 1.0, self.steps)):
            ...
            xs_tensor_mod = tensor(xs_mod)
            with tf.GradientTape() as intgrad_tape:
                intgrad_tape.watch(xs_tensor_mod)
                y_pred = self.model(xs_tensor_mod)
            gradients = intgrad_tape.gradient(y_pred, xs_tensor_mod)
            _attr = gradients
            if gradient is None: gradient = _attr
            else: gradient = [g + a for g, a in zip(gradient, _attr)]
...
class EpsilonLRP(GradientBasedMethod):
    eps = None
    def __init__(self, T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase, epsilon=1e-4):
        super(EpsilonLRP, self).__init__(T, xs, xs_tensor, y, tape_obejct, model, keras_learning_phase)
...
class DeepExplain(object):
    ...
    def explain(self, method, T, xs, xs_tensor, y, tape_obejct, model, **kwargs):
        ...
        method = _ENABLED_METHOD_CLASS(T, xs, xs_tensor, self.y, self.tape, self.model, self.keras_phase_placeholder, **kwargs)
        result = method.run()
        if issubclass(_ENABLED_METHOD_CLASS, GradientBasedMethod) and _GRAD_OVERRIDE_CHECKFLAG == 0:
            warnings.warn(Fore.LIGHTYELLOW_EX + 'DeepExplain detected you are trying to use an attribution method that requires '
                          'gradient override but the original gradient was used instead. You might have forgot to '
                          '(re)create your graph within the DeepExlain context. Results are not reliable!' + Fore.RESET)
        _ENABLED_METHOD_CLASS = None
        _GRAD_OVERRIDE_CHECKFLAG = 0
        self.keras_phase_placeholder = None
        return result
